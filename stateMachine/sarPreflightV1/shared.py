...
class SharedStatus:
    ...
    Status (written by state machine, read by GUI):
        current_state   : "WAIT", "CHANGE_MODE", etc.
    ...
    Commands (written by GUI, read by state machine):
        pending_command  : mode name to switch to, or None
    """
    ...
    def send_command(self, command):
        ...
        Commands are dicts with a "type" field:
            {"type": "mode_change", "mode": "LOITER"}
            {"type": "timed_hold", "mode": "LOITER", "duration": 10}

        For backwards compatibility, a plain string is treated as
        a mode change.
        """
        with self._lock:
            if isinstance(command, str):
                command = {"type": "mode_change", "mode": command}
