...
class DroneConnection:
    ...
    def connect(self):
        ...
        mission_log.connected(self.conn.target_system, self.conn.target_component)
        ...
        for msg_id, interval_us in [
            (mavutil.mavlink.MAVLINK_MSG_ID_GPS_RAW_INT,         250000),
            (mavutil.mavlink.MAVLINK_MSG_ID_SYS_STATUS,          500000),
            (mavutil.mavlink.MAVLINK_MSG_ID_GLOBAL_POSITION_INT, 250000),
    ...
    def get_mode(self, retries=3):
        """
        Read the current flight mode from the autopilot heartbeat.

        Primary path (fast — used during normal state loop ticks):
            drain_buffer() must have been called first on this tick.
            get_mode() then reads the autopilot heartbeat from the
            pymavlink message cache (self.conn.messages["HEARTBEAT"]).
            This is instantaneous — no blocking wait — so the loop
            tick is not delayed and telemetry stays current.

        Fallback path (blocking — used if cache is empty):
            If no heartbeat is in the cache yet (e.g. first call after
            connect), falls back to recv_match with a short timeout.
            Retries up to `retries` times before returning UNKNOWN.

        Filtering:
            Only accepts heartbeats from the autopilot component
            (sys=target_system, comp=1).  Ignores ADSB, GCS etc.
        """
        # ── Fast path: read from cache ──
        hb = self.conn.messages.get("HEARTBEAT")
        if hb is not None:
            if (hb.get_srcSystem() == self.conn.target_system and
                    hb.get_srcComponent() == self.conn.target_component):
                mode_num  = hb.custom_mode
                mode_name = MODE_NAMES.get(mode_num, f"MODE_{mode_num}")
                if self.status:
                    self.status.set_mode(mode_name)
                mission_log.mode_change(mode_name)
                return mode_name, mode_num

        # ── Fallback: blocking recv_match ──
        # Cache is empty (startup) or last drain produced no autopilot HB.
    ...
    def drain_buffer(self, max_msgs=50):
        """
        Pump the MAVLink receive queue so pymavlink's message cache
        stays fresh for ALL message types.

        MUST be called at the top of every state loop tick, before
        get_mode() or get_telemetry().

        Why this is necessary:
            recv_match(type="HEARTBEAT") discards non-heartbeat
            packets rather than caching them.  Without an unfiltered
            drain pass, GPS_RAW_INT, SYS_STATUS, GLOBAL_POSITION_INT
            etc. never reach self.conn.messages, so get_telemetry()
            always returns None for GPS and battery fields.

        This call is non-blocking and returns immediately once the
        queue is empty, so it adds negligible latency.
        """
    ...
    def get_telemetry(self):
        ...
        Returns a dict using pymavlink's cached messages.
