...
Before transitioning here, the caller must set:
    ...
    shared["return_to"] — state to return to (default: "WAIT")
...
CRITICAL DESIGN NOTE — Why all fences are uploaded together:
    ...
    are stored in shared["base_fence_polygons"] so that REPLAN
    can re-upload them alongside the new Focus Area fence.
...
class UploadFenceState(BaseState):
    ...
    def execute(self):
        ...
        # But DO store base fences for later re-upload during REPLAN.
