...
Updates SharedStatus so the GUI stays in sync.
"""
...
class DroneConnection:
    ...
    def connect(self):
        ...
        # Private heartbeat cache — only updated from the real autopilot.
        # pymavlink's self.conn.messages["HEARTBEAT"] is overwritten by
        # ANY heartbeat source (including MAVProxy GCS, ADSB peripherals),
        # so we cannot trust it for mode reads.
        ...
        # When connecting directly to SITL or a Cube without MAVProxy,
        # the autopilot does not stream GPS_RAW_INT, SYS_STATUS etc.
        # unless explicitly requested.  MAVProxy normally handles this
        # automatically; we must do it ourselves here.
        self.log("INIT", "Requesting telemetry streams ...")
        ...
        # Also explicitly request the specific messages we need,
        # in case MAV_DATA_STREAM_ALL is not honoured (common in ArduPilot 4.x+)
        for msg_id, interval_us in [
            (mavutil.mavlink.MAVLINK_MSG_ID_GPS_RAW_INT,        250000),  # 4 Hz
        ...
        self.log("INIT", "Stream requests sent")
    ...
    def get_mode(self, retries=3):
        """
        Read current flight mode from the cached autopilot heartbeat.

        Fast path (normal operation):
            Reads self.conn.messages["HEARTBEAT"] which is populated by
            drain_buffer().  This is a dict lookup — effectively instant.
            Call drain_buffer() BEFORE get_mode() on every loop tick.

        Slow path (startup only):
            If the cache is empty (first call before any heartbeat has
            arrived), falls back to a blocking recv_match with retries.
            This only happens once at startup.

        Performance:
            The old implementation used blocking recv_match(type=
            "HEARTBEAT") on every call, which throttled the loop to
            ~1 Hz (heartbeat rate).  The cache-read approach lets the
            monitoring loop run at full speed (<100 ms per tick).
        """
        # ── Fast path: read from filtered autopilot-only cache ─────
        # self._autopilot_hb is updated by drain_buffer() and only ever
        # holds heartbeats from the real autopilot (target_system/component).
        # pymavlink's self.conn.messages["HEARTBEAT"] is NOT safe — it is
        # overwritten by MAVProxy's own heartbeat (custom_mode=0 → STABILIZE).
        ...
        # ── Slow path: blocking fallback (startup / no cache yet) ─
    ...
    def set_mode(self, mode_name):
        ...
        while time.time() - start < MODE_CHANGE_TIMEOUT:
            ...
            if hb.custom_mode == target_id:
                # Update the filtered autopilot cache so get_mode() on
                # the very next tick sees the new mode immediately.
                # Without this, _autopilot_hb holds the OLD heartbeat
                # and the first monitor tick fires a false override.
    ...
    # ── Landing mission upload ────────────────────────────────

    def upload_landing_mission(self, target_lat, target_lon,
                               approach_alt, speed_mps, timeout=15):
        """
        Upload a short AUTO mission that flies to a target waypoint
        at a given altitude and then lands there.

        Used by DELIVER phase 4 instead of GUIDED + SET_POSITION_TARGET
        to take advantage of the robust AUTO mission protocol (no single
        packet can be lost mid-flight — ArduPilot owns the sequence).

        Items:
            Seq 0 — HOME placeholder (NAV_WAYPOINT, ArduPilot overwrites)
            Seq 1 — DO_CHANGE_SPEED        (ground speed, m/s)
            Seq 2 — NAV_WAYPOINT           (target_lat, target_lon, approach_alt)
            Seq 3 — NAV_LAND               (target_lat, target_lon, 0)

        Returns True on ACK, False on timeout or error.
        """
        items = [
            # Seq 0 — HOME placeholder.  Coordinates don't matter.
            {
                "seq": 0,
                "frame": mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
                "command": mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
            ...
            # Seq 1 — DO_CHANGE_SPEED (ground speed, m/s, no throttle change)
            {
                "seq": 1,
                "frame": mavutil.mavlink.MAV_FRAME_MISSION,
                "command": mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED,
                "p1": 1,                     # speed type: ground speed
                "p2": float(speed_mps),      # speed (m/s)
                "p3": -1,                    # throttle: no change
                "p4": 0,
                "lat": 0.0, "lon": 0.0, "alt": 0.0,
            },
            # Seq 2 — NAV_WAYPOINT to target at approach altitude
            {
                "seq": 2,
                "frame": mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
                "command": mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
                ...
                "p3": 0,                     # pass-through radius
                "p4": 0,                     # desired yaw (NaN/0 = any)
                "lat": float(target_lat),
                "lon": float(target_lon),
                "alt": float(approach_alt),
            },
            # Seq 3 — NAV_LAND at target
            {
                "seq": 3,
                "frame": mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
                "command": mavutil.mavlink.MAV_CMD_NAV_LAND,
                "p1": 0,                     # abort altitude (0 = default)
                "p2": 0,                     # precision land (0 = off)
                "p3": 0,
                "p4": 0,                     # desired yaw
                "lat": float(target_lat),
                "lon": float(target_lon),
        ...
        count = len(items)
        self.log("MISSION", f"Uploading landing mission ({count} items) — "
                 f"target={target_lat:.6f},{target_lon:.6f} "
                 f"approach_alt={approach_alt}m speed={speed_mps}m/s")
        ...
        # ── Step 2 & 3: Respond to MISSION_REQUEST(_INT) ──
        deadline = time.time() + timeout
        while time.time() < deadline:
            msg = self.conn.recv_match(
                type=["MISSION_REQUEST_INT", "MISSION_REQUEST", "MISSION_ACK"],
            ...
            if msg_type == "MISSION_ACK":
                # Ignore fence ACKs that may race through.
                if hasattr(msg, "mission_type") and msg.mission_type != 0:
                    continue
                result = msg.type
                if result == mavutil.mavlink.MAV_MISSION_ACCEPTED:
                    self.log("MISSION", "Landing mission upload complete — ACK received")
                    return True
                self.log("MISSION", f"Landing mission REJECTED — ACK type {result}")
                return False

            if msg_type in ("MISSION_REQUEST_INT", "MISSION_REQUEST"):
                # Ignore fence requests
                if hasattr(msg, "mission_type") and msg.mission_type != 0:
                    continue
                seq = msg.seq
                if seq < 0 or seq >= count:
                ...
                self.log("MISSION", f"  Sent landing item seq={seq} cmd={item['command']}")

        self.log("MISSION", "Landing mission upload TIMEOUT — no ACK received")
        return False
    ...
    def set_parameter_nowait(self, param_name, value, param_type=None):
        """
        Fire-and-forget parameter set — sends PARAM_SET without waiting
        for the PARAM_VALUE echo.

        Use this for speed changes during flight where blocking for 15+
        seconds is unacceptable.  The parameter almost always takes
        effect immediately on ArduPilot even without confirmation; the
        echo is just diagnostic.

        For critical pre-flight parameters (fence, etc.) use the
        blocking set_parameter() instead.
        """
        ...
        self.log("PARAM", f"Sent {param_name} = {value} (no-wait)")
    ...
    def drain_buffer(self):
        """
        Pump the MAVLink receive queue until it is empty, so pymavlink's
        message cache stays fresh for ALL message types.

        MUST be called at the top of every state loop tick, before
        get_mode() or get_telemetry().

        Why this is necessary:
            get_mode() reads from self._autopilot_hb (a filtered cache
            that only stores heartbeats from the real autopilot).
            Without drain_buffer() the cache never gets updated and
            get_mode() returns stale data.

            Similarly, GPS_RAW_INT, SYS_STATUS, GLOBAL_POSITION_INT,
            MISSION_CURRENT, MISSION_ITEM_REACHED etc. only reach
            self.conn.messages if they are consumed from the receive
            queue by an unfiltered recv_match call.

        No message cap: in SITL, ArduPilot streams 10+ message types
        at 5–10 Hz, producing 100–200 messages per 2-second tick.
        A fixed cap (e.g. 50) leaves the buffer partially read and
        mission-tracking messages (MISSION_CURRENT) get stranded
        behind the high-rate telemetry flood, causing stale waypoint
        progress in the GUI.

        recv_match(blocking=False) returns None immediately when the
        queue is empty, so this loop adds negligible latency.
        """
        while True:
            msg = self.conn.recv_match(blocking=False)
            if msg is None:
                break
            # Keep a private heartbeat cache filtered to the AUTOPILOT only.
            # pymavlink's self.conn.messages["HEARTBEAT"] is overwritten by
            # ANY source — including MAVProxy (sys=255, custom_mode=0 →
            # STABILIZE) which causes phantom mode readings.
    ...
    def get_telemetry(self):
        ...
        consume from the live buffer.

        Requires drain_buffer() to have been called first on this
        loop tick, otherwise the cache will not contain fresh data.
        """
        telem = {
            "alt": None,
            "lat": None,
    ...
    def get_armed(self):
        """
        Check if the vehicle is armed from the last cached autopilot heartbeat.
    ...
    def query_mission_count(self, timeout=5):
        ...
        Used by SearchState to verify the mission is on the autopilot
        before entering AUTO mode.  upload_mission.py pops shared["waypoints"]
        after upload, so this is the only reliable way to confirm.
