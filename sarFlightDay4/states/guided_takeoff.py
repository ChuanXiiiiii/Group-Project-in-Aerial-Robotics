...
==================================================
Arms the drone, commands a GUIDED mode takeoff to SEARCH_ALT_M,
confirms the target altitude is reached, then returns to IDLE.
The drone remains armed in GUIDED mode, ready for the operator
to open the Pre-AUTO checklist.
...
class GuidedTakeoffState(BaseState):
    ...
    def _wait_for_altitude(self, target_m, tolerance_m, timeout_s):
        ...
        Logs to console every ~5 seconds; pushes GUI status every ~1s.

        Uses a short 0.25s sleep between cache reads — drain_buffer()
        keeps the message cache fresh, so there is no benefit in
        sleeping longer than needed for responsive altitude tracking.
        """
        deadline = time.time() + timeout_s
        last_log = 0.0
        last_push = 0.0
        while time.time() < deadline:
            ...
            if alt is not None:
                if abs(alt - target_m) <= tolerance_m:
                    return True
                now = time.time()
                # Push GUI update every ~1s for responsive altitude display
                if now - last_push >= 1.0:
                    self._push_status(f"CLIMBING {alt:.1f}m")
                    last_push = now
                # Console log every ~5s to avoid spam
                if now - last_log >= 5.0:
