...
class PreAutoCheckState(BaseState):
    ...
    def _evaluate_checks(self, mode_name, hb_ok):
        ...
        # ── 5. Mode = GUIDED ──
        # The RC switch is LOITER / GUIDED / LOITER.  The pilot signals
        # "ready to hand over" by placing the switch in GUIDED (centre
        # position).  We only allow START SEARCH when in GUIDED — this
        # guarantees the pilot has deliberately consented to autonomous
        # flight.  If the switch is in LOITER, the pilot has not yet
        # handed over and the state machine must not request AUTO.
        mode_pass = (mode_name == "GUIDED")
        ...
        # wp_verified removed from GUI — auto-pass so it doesn't block the gate
        wp_verified  = True
        ...
        all_ready = all([
            ...
            alt_pass, mission_pass, fence_pass, wp_verified, pilot_ready,
        ])
        ...
        return {
            ...
            "mode":        {"pass": mode_pass,
                            "detail": f"{mode_name} (need GUIDED)"},
            "armed":       {"pass": arm_pass,
