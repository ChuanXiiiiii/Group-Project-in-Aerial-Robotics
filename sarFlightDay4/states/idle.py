...
class IdleState(BaseState):
    ...
    def execute(self):
        while True:
            ...
            # ── 2. Store home position (once, when GPS is valid) ────
            if not self.shared.get("home_stored") and hb_ok:
                telem = self.drone.get_telemetry()
                if (telem.get("lat") and telem.get("lon")
                        and telem.get("gps_fix") is not None
                        and telem.get("gps_fix") >= 3):
                    self.shared["home_lat"] = telem["lat"]
                    self.shared["home_lon"] = telem["lon"]
                    self.shared["home_stored"] = True
                    self.log(f"Home position stored: "
                             f"{telem['lat']:.6f}, {telem['lon']:.6f}")

            # ── 3. Push live telemetry to GUI ─────────────────────────
            self._push_idle_status()
